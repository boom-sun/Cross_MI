...
def get_model(args, device, config, ti, X, y, current_path, load_classifier=False):
    ...
    if config['backbone'].lower() == 'eegnet':
        model = EEGNet(n_channels=X.shape[1],
                        n_samples=X.shape[2],
                        n_classes=n_classes)
...
class DeepConvNet(nn.Module):
    def __init__(self, num_ch=21, drop=None):
