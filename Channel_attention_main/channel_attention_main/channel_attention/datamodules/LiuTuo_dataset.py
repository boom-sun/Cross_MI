...
from utils.load_LiuTuo import load_Liu_data

class LiuTuo(BaseDataModule):
    all_subject_ids = list(range(1, 11))
    class_names = ["feet", "rest"]

    def __init__(self, preprocessing_dict, subject_id, train_rate):
        super().__init__(preprocessing_dict, subject_id, train_rate)

    def prepare_data(self) -> None:
        self.dataset = load_Liu_data([self.subject_id], self.preprocessing_dict, self.train_rate)
