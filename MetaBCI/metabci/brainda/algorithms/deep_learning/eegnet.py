...
class EEGNet(nn.Module):
    ...
    def __init__(self, n_channels, n_samples, n_classes):
        ...
        self.features = nn.Sequential(self.step1, self.step2, self.step3)
        self.classifier = nn.Sequential(self.fc_layer)
...
class EEGNetManager:
    ...
    @classmethod
    def load(cls, model_class, config_path, weights_path, device='cpu'):
        """
        加载模型，返回 EEGNetManager 实例
        """
        with open(config_path, "r") as f:
            init_params = json.load(f)
        instance = cls(model_class, init_params)
        instance.model.load_state_dict(torch.load(weights_path, map_location=device))
        instance.model.eval()
        return instance
