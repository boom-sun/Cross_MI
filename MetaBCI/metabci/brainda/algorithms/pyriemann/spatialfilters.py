...
def safe_log(arr, method='clip', fill_value=0):
    """
    安全计算对数，避免NaN值

    参数:
    arr: 输入数组
    method: 处理方法 ('clip', 'where', 'mask' 或 'offset')
    fill_value: 当使用'where'方法时，用于填充非正值的值

    返回:
    处理后的对数数组
    """
    if method == 'clip':
        min_val = np.finfo(arr.dtype).eps
        safe_arr = np.clip(arr, min_val, None)
        return np.log(safe_arr)

    elif method == 'where':
        return np.where(arr > 0, np.log(arr), fill_value)

    elif method == 'mask':
        masked_arr = np.ma.masked_where(arr <= 0, arr)
        return np.ma.log(masked_arr)

    elif method == 'offset':
        epsilon = 1e-12
        safe_arr = arr + epsilon
        return np.log(safe_arr)

    else:
        raise ValueError("方法必须是 'clip', 'where', 'mask' 或 'offset'")
...
class CSP_sun(BilinearFilter):

    def __init__(self, nfilter=3, metric='euclid', log=True):
        """Init."""
        self.nfilter = nfilter
        self.metric = metric
        self.log = log
    ...
    def fit(self, X, y):
        ...
        # spatial patterns
        A = np.linalg.pinv(evecs.T)  # np.linalg.pinv求逆

        self.filters_ = evecs[:, 0:self.nfilter].T
        self.filters_ = np.concatenate((self.filters_,
                                        evecs[:, evecs.shape[0]-self.nfilter:evecs.shape[0]].T))
        self.patterns_ = A[:, 0:self.nfilter].T
