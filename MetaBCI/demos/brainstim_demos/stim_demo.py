...
import numpy as np
from MetaBCI.metabci.brainstim.paradigm import SSVEP,P300,MI,AVEP,paradigm
from MetaBCI.metabci.brainstim.framework import Experiment
...
if __name__=='__main__':
    ...
    avep.config_pos(n_elements=n_elements, rows=rows, columns=columns, stim_length=stim_length, stim_width=stim_width)
    avep.config_color(refresh_rate=fps, stim_time=stim_time, stimtype='sinusoid',
                      stim_color=stim_color, sequence=sequence,
                      stim_opacities=stim_opacities, freqs= np.ones((n_elements)) * freqs, stim_num=stim_num)
    ...
    ex.register_paradigm('avep', paradigm, VSObject=avep, bg_color=bg_color, display_time=display_time,
                         ...
                         nrep=nrep,
                         pdim='avep', lsl_source_id=lsl_source_id, online=online)
