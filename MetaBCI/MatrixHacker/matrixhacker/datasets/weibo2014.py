...
WEIBO_URL = 'C:\\Users\\Administrator\\PycharmProjects\\python310\\MrLiuTuo\\MNE_weibo_2014'
...
class Weibo2014(BaseDataset):
    ...
    def _get_single_subject_data(self, subject, verbose=False):
        ...
        ch_names = self._CHANNELS + ['VEO', 'HEO', 'STI 014']
        ...
        # 0.19.2 can not guarante channel name case insensitive  0.19.2不能保证通道名不区分大小写
    ...
    @verbose
    def _get_all_subject_data(self, verbose=False):
        """return data for a single subject"""
        for subject in range(1,10):
            ...
            stim = np.zeros((1, epoch_data.shape[1], epoch_data.shape[2]))
            stim[0, 0, :] = label
            single_data = np.concatenate((epoch_data, stim), axis=0)
            single_data = np.transpose(single_data, axes=(0, 2, 1))
            single_data = np.reshape(single_data, (single_data.shape[0], -1))
            if subject==1:
                data=single_data
            else:
                data=np.concatenate((data,single_data),axis=1)  
        ch_names = self._CHANNELS + ['VEO', 'HEO', 'STI 014']
        ch_names.insert(57, 'CB1')
        ch_names.insert(61, 'CB2')
        ch_types = ['eeg']*62 + ['eog']*2
