...
class OnlineConsecutiveFilter(BaseEstimator, TransformerMixin):
    ...
    def _compute_filter(self, l, h, fs, order=5, return_sos=True):
...
class OnlineBlockFilter(BaseEstimator, TransformerMixin):
    ...
    def _compute_filter(self, l, h, fs, order=3, return_sos=True):
        ...
        ----------
        l: float|无
        低截止频率，如果为None则返回低通滤波器系数
        h: float|无
        高截止频率，如果为None则返回高通滤波器系数
        Fs:浮动
        采样率
        秩序:int
        巴特沃斯滤波器阶
        Return_sos: bool，可选
        如果为True，返回sos coeffs，默认为True
